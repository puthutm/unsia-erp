  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-dark-900">Akademik</h1>
          <p className="text-dark-500 mt-1">Kelola kegiatan akademik</p>
        </div>
        <div className="flex gap-2">
          <select className="px-3 py-2 border border-surface-border rounded-lg text-dark-600">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-6 border border-surface-border">
          <h3 className="text-sm font-medium text-dark-500">Total Mahasiswa</h3>
          <p className="text-3xl font-bold text-dark-900 mt-2">{students.length}</p>
        </div>
        <div className="bg-white rounded-xl p-6 border border-surface-border">
          <h3 className="text-sm font-medium text-dark-500">Aktif</h3>
        ...
        <div className="bg-white rounded-xl p-6 border border-surface-border">
          <h3 className="text-sm font-medium text-dark-500">Program Studi</h3>
          <p className="text-3xl font-bold text-dark-900 mt-2">{studyPrograms.length}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-surface-border">
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center text-dark-500 py-8">Tidak ada mahasiswa</div>
          ) : activeTab === "students" ? (
            <table className="w-full">
              <thead className="bg-surface-subtle">
                <tr>
                  ...
                  <th className="text-left p-4 text-sm font-medium text-dark-500">Tahun Masuk</th>
              ...
              <tbody>
                {students.slice(0, 10).map((student) => (
                  <tr key={student.id} className="border-t border-surface-border">
                    <td className="p-4 text-dark-900 font-mono">{student.nim}</td>
                    <td className="p-4 text-dark-900">{student.name}</td>
                    <td className="p-4 text-dark-600">{student.studyProgramName}</td>
                    <td className="p-4 text-dark-600">{student.entryYear}</td>
                    <td className="p-4">
            ...
            <div className="text-center text-dark-500 py-8">Tidak ada jadwal</div>
          ) : activeTab === "schedules" ? (
            <table className="w-full">
              <thead className="bg-surface-subtle">
                <tr>
                  ...
                  <th className="text-left p-4 text-sm font-medium text-dark-500">Hari</th>
                  <th className="text-left p-4 text-sm font-medium text-dark-500">Jam</th>
                  <th className="text-left p-4 text-sm font-medium text-dark-500">Ruang</th>
                  <th className="text-left p-4 text-sm font-medium text-dark-500">Dosen</th>
                </tr>
              </thead>
              <tbody>
                {schedules.slice(0, 10).map((schedule) => (
                  <tr key={schedule.id} className="border-t border-surface-border">
                    <td className="p-4 text-dark-900">{schedule.courseName}</td>
                    <td className="p-4 text-dark-600">{schedule.className}</td>
                    <td className="p-4 text-dark-600">{schedule.day}</td>
                    <td className="p-4 text-dark-600">{schedule.startTime} - {schedule.endTime}</td>
                    <td className="p-4 text-dark-600">{schedule.room}</td>
                    <td className="p-4 text-dark-600">{schedule.lecturerName}</td>
                  </tr>
                ))}
              </tbody>
            </table>
