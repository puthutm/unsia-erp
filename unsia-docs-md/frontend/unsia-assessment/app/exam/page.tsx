...
// Types for exam question/answer flow
...
function ExamRunner() {
  ...
  // Local state for exam flow - needed for the exam runner
  const [currentAttempt, setCurrentAttempt] = useState<ExamAttempt | null>(null);
  const [questions, setQuestions] = useState<ExamQuestion[]>([]);
  const [answers, setAnswers] = useState<ExamAnswers>({});
  const [isLoading, setIsLoading] = useState(false);

  // Initialize exam session when sessionId is available
  useEffect(() => {
    if (sessionId) {
      setIsLoading(true);
      // Setup mock exam data for the session
      const mockQuestions: ExamQuestion[] = [
        { id: "q1", text: "Apa ibu kota Indonesia?", options: [{ key: "A", text: "Jakarta" }, { key: "B", text: "Bandung" }, { key: "C", text: "Surabaya" }, { key: "D", text: "Medan" }] },
        { id: "q2", text: "2 + 2 = ?", options: [{ key: "A", text: "3" }, { key: "B", text: "4" }, { key: "C", text: "5" }, { key: "D", text: "6" }] },
      ];
  ...
  const startAttempt = async (sId: string) => {
  ...
  const [timeLeft, setTimeLeft] = useState(3600); // 60 mins fallback
  ...
  if (isLoading || !currentAttempt || questions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-4">
        ...
        <p className="text-slate-500 font-medium text-sm">Menyiapkan ruang ujian CBT... Mohon tunggu.</p>
      </div>
    );
  }
